...
async function runQueries(){
  while(state.pendingQuery){
    ...
    try{
      ...
      if(q)$('filterBanner').innerHTML=`<div><strong>有效测量视图 · 过滤 v${q.version}</strong><span>${q.total.toLocaleString()} 条原始采样；运动/静止按三轴合成峰值偏差 ≤ ${number(MOTION_THRESHOLD_G,3)} g / &gt; ${number(MOTION_THRESHOLD_G,3)} g 判定。${q.anomaly_samples.toLocaleString()} 条含异常字段，${(q.status_samples||0).toLocaleString()} 条有导航状态提示；状态提示不屏蔽参数，只有失真速度和明显位置跳点被隔离。${q.pending_samples?` ${q.pending_samples} 条待判定，测量暂不展示。`:''}</span></div><button class="text-button" data-goto="quality">查看剔除原值 ↗</button>`;
...
function renderOverview(){
  ...
  const cards=[['估算运行里程',number(s.distance_km,2),'km','仅三轴判定为运行且定位连续'],['有效运动时间',number(s.moving_s/60,1),'min',`三轴峰值偏差 > ${number(MOTION_THRESHOLD_G,3)} g`],['最高有效速度',number(s.max_kmh,2),'km/h','剔除异常速度和位置跳点'],['RTK 固定解占比',number(s.fixed_pct,1),'%','固定解 ≠ 精度承诺'],['异常 / 质量事件',String(d.events.total),'项',`${s.gap_count} 处数据时间断档`]];
...
const chartGroups=[
 ...
 ['车辆平面速度','km/h',[['speed','车辆速度','km/h',3.6]],'载体相对地面的速度，换算为 km/h。'],
...
function renderFilterSummary(){
  ...
  const scopes=q.contexts.map(c=>`<article class="stationary-reference"><div><span class="status-pill good">历史静止记录（仅审计）</span><strong>SN ${esc(c.device_id)}</strong><p>${sampleStamp(c.start)} — ${c.active?'至今':sampleStamp(c.end)} · 北京时间</p><p>v6 已改用三轴峰值偏差判定；该旧静止上下文不再屏蔽运行或静止位置。</p></div></article>`).join('');
  $('filterSummary').innerHTML=`<div class="quality-grid filter-counts">${cards.map(([label,value,note])=>`<section class="panel"><h3>${label}</h3><strong>${value.toLocaleString()}</strong><p>${note}</p></section>`).join('')}</div>${scopes||'<p class="scope-note">当前时段没有生效的静止上下文。三轴合成峰值偏差 ≤ 0.01 g 判为静止，> 0.01 g 判为运行；静止期间保留导航模式位置。</p>'}<p class="scope-note">状态提示不等同于被剔除字段；明显位置跳点只屏蔽经纬度，速度上限异常只隔离导航字段，IMU 与原始采样继续保留。${q.pending_samples?`${q.pending_samples} 条待判定，测量暂不展示。`:''}查看和导出采用页面上方已查询的设备及时间范围。</p>`;
