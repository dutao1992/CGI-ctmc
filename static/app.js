...
function renderFilterSummary(){
  ...
  const scopes=q.contexts.map(c=>{
    ...
    const lifecycle=c.active
      ? `后续采样使用静止参考。系统仅在组合导航、RTK 固定/浮点、定位 σ ≤ ${number(policy?.max_position_std_m,1)} m，且离锚点 ≥ ${number(policy?.min_anchor_distance_m,0)} m后持续运动 ${number(policy?.min_duration_s,0)} 秒时自动退出；出发前仍应主动关闭。`
