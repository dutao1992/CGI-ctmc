...
function renderFilterSummary(){
  ...
  const scopes=q.contexts.map(c=>{
    const automatic=c.closure?.action==='quality.stationary_context.auto_close',policy=c.auto_exit_policy;
    const status=c.active?'持续静止 · 自动防护':automatic?'已自动恢复运动规则':'用户确认静止';
    const lifecycle=c.active
      ? `后续采样使用静止参考。系统仅在组合导航、RTK 固定/浮点定位、定位 σ ≤ ${number(policy?.max_position_std_m,1)} m，原始速度 ≥ ${number(policy?.min_speed_ms,2)} m/s，离锚点 ≥ ${number(policy?.min_anchor_distance_m,0)} m后持续运动 ${number(policy?.min_duration_s,0)} 秒、净位移 ≥ ${number(policy?.min_displacement_m,0)} m且轨迹有效率 ≥ ${number((policy?.min_path_efficiency||0)*100,0)}% 时自动退出；出发前仍可主动关闭。`
