...
function navigate(view, updateHash = true) {
  ...
  setTimeout(() => {
    ...
    if (["board", "master"].includes(view)) ensureWorkspaceDetails().then(() => { if(view === "board")renderAnalyticsBoard(); }).catch((error) => toast(error.message));
  }, 100);
  return true;
}

let workspaceDetailsRequest = null;
