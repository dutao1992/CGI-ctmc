...
const state = {devices:[],device:null,data:null,view:'overview',range:3600,rangeMode:'relative',pendingQuery:null,queryPromise:null,activeQueryController:null,lastQueriedDeviceT:null,canManage:false,request:0,playing:false,playT:0,charts:[],chartCache:new Map(),signalRenderRevision:0,health:null,qualityOffset:0,qualityKey:null,qualityRequest:0,offlineFile:null,offlineData:null,offlineChartCache:new Map()};
const rules = {
...
function clearResults(message){
  ...
  for(const id of ['kpis','segments','eventPreview','eventSummary','eventTop10','eventList','filterSummary','quarantineList'])$(id).innerHTML=`<div class="empty">${esc(message)}</div>`;
...
setInterval(async()=>{if(document.hidden||refreshing||state.queryPromise)return;refreshing=true;try{await refreshHealth();const status=await loadDevices(false,state.view==='fleet');const latest=status.device?.last_t??null;const newSamples=latest!==state.lastQueriedDeviceT;if(newSamples&&state.rangeMode==='relative'&&$('autoRefresh').checked&&!state.playing&&!$('pointDialog').open&&!$('stationaryDialog').open&&!$('queryBtn').disabled&&!['fleet','offline'].includes(state.view)){setRange(state.range);await query({relative:true});}}catch(e){error(e.message);}finally{refreshing=false;}},15000);
