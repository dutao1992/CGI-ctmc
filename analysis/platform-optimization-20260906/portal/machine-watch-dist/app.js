...
async function loadData() {
  const response = await fetch(`./data/machine_watch_data.json`, { cache: "no-cache" });
...
function renderSummary() {
  ...
  const validationText = validation
    ...
    : "数据已载入";
  el("generatedAt").textContent = `${validationText} · 数据快照 ${new Date(dataset.generatedAt).toLocaleString("zh-CN")}`;
}
...
el("refreshBtn").setAttribute("aria-label", "数据快照：线上页面展示最近一次校验通过的数据");
}
