...
def list_analyses(q: str = "") -> list[dict]:
    with db() as conn:
        if q.strip():
            pattern = f"%{q.strip()}%"
            rows = conn.execute(
                """SELECT * FROM analyses
                WHERE case_no LIKE ? OR part_name LIKE ? OR machine_model LIKE ?
                   OR machine_position LIKE ? OR metadata_json LIKE ? OR report_json LIKE ?
                ORDER BY created_at DESC LIMIT 100""",
                (pattern, pattern, pattern, pattern, pattern, pattern),
            ).fetchall()
        else:
            rows = conn.execute("SELECT * FROM analyses ORDER BY created_at DESC LIMIT 100").fetchall()
    return [row_to_item(row, compact=True) for row in rows]
