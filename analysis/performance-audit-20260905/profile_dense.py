...
with s.connect() as c:
    rows=c.execute('SELECT bucket_start,point_count FROM point_rollups WHERE device_id=? AND bucket_s=60 ORDER BY bucket_start',(d['id'],)).fetchall()
    window=collections.deque();n=0;best={'n':-1}
    for row in rows:
        window.append(row);n+=row['point_count']
        while window and window[0]['bucket_start']<=row['bucket_start']-3600:n-=window.popleft()['point_count']
        if n>=best['n']:best={'bucket_start':row['bucket_start'],'n':n}
end=best['bucket_start']+60;sn=d['id']
